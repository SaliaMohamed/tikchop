...
export default function ShopInfoPage() {
  ...
  const brandingReady = Boolean(profile.logo_url || profile.brand_color !== "#059669" || profile.physical_address);
    ...
    return (
      <div className="app-shell flex min-h-[60vh] items-center justify-center">
        <div className="text-center">
          <Loader2 className="mx-auto animate-spin text-[#059669]" size={30} />
          <p className="mt-3 text-sm font-black text-[#0F2B20]/50">Chargement boutique...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="app-shell pb-[calc(7rem+env(safe-area-inset-bottom,0px))] md:pb-8">
      <header className="overflow-hidden rounded-[26px] bg-[#0F2B20] p-5 text-white shadow-[0_4px_28px_rgba(15, 43, 32,0.25)] md:p-7">
        <div className="grid gap-5 lg:grid-cols-[1fr_120px_280px] lg:items-center">
          <div>
            <p className="text-[0.68rem] font-black uppercase tracking-[0.14em] text-[#34D399]/80">Informations boutique</p>
          ...
          <div className="rounded-[20px] bg-white/8 p-4 ring-1 ring-white/10">
            <p className="text-[0.68rem] font-black uppercase tracking-[0.14em] text-[#34D399]/70">Fiche complete</p>
            ...
            <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/12">
              <div className="h-full rounded-full bg-[#34D399] transition-all duration-700" style={{ width: `${readiness}%` }} />
            </div>
          </div>
        </div>
      </header>
          ...
          <section className="overflow-hidden rounded-[26px] bg-[#F6FBF7] ring-1 ring-[#0F2B20]/10">
            ...
            <div className="p-4 grid gap-3 md:grid-cols-2">
              ...
              <Field label="Lien public">
                <div className="flex min-h-[54px] overflow-hidden rounded-[18px] bg-[#F6FBF7] ring-1 ring-[#0F2B20]/12">
                  <input value={publicUrl} readOnly className="min-w-0 flex-1 bg-transparent px-4 text-sm font-black text-[#0F2B20] outline-none" />
                  <button type="button" onClick={copyPublicUrl} className="flex w-11 items-center justify-center border-l border-[#0F2B20]/10 text-[#059669]" aria-label="Copier">
                    <Copy size={17} />
                  </button>
                </div>
              </Field>
            ...
            <div className="p-4 space-y-4">
              <div className="grid gap-4 md:grid-cols-[140px_1fr]">
                <div className="flex flex-col items-center justify-center">
                  <span className="text-xs font-black uppercase tracking-wider text-[#54685E]/80 mb-2">Logo</span>
                  <input
                  ...
                  <button
                    ...
                    className="relative flex h-24 w-24 items-center justify-center overflow-hidden rounded-full border-2 border-dashed border-[#DCEFE3] bg-white text-center hover:bg-[#F6FBF7] transition"
                  >
                    {profile.logo_url ? (
                      <>
                    ...
                    {logoUploading && (
                      <span className="absolute inset-0 flex items-center justify-center bg-white/70 text-[#059669]">
                  ...
                  <Field label="Couleur de marque">
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className="h-11 w-16 shrink-0 cursor-pointer rounded-xl border border-[#0F2B20]/12 p-1 bg-white"
                      />
                      <input
                        type="text"
                        value={profile.brand_color}
        ...
        <aside className="space-y-4">
            ...
            <div className="p-3 space-y-2">
              {zones.slice(0, 6).map((zone) => (
                <div key={zone.id} className="flex items-center justify-between rounded-[16px] bg-white px-3 py-2.5 ring-1 ring-[#0F2B20]/8">
                  <span className="text-sm font-black text-[#0F2B20]">{zone.name}</span>
                  <span className="text-xs font-black text-[#059669]">{formatPrice(zone.fee)}</span>
                </div>
              ))}
          ...
          <section className="overflow-hidden rounded-[26px] bg-[#0F2B20] text-white">
            <div className="border-b border-white/10 px-4 py-3">
              <p className="text-[0.68rem] font-black uppercase tracking-[0.12em] text-[#34D399]/80">Apercu bot</p>
...
function InfoStatusCard({ icon, title, ok, text, href, action }) {
  return (
    <section className={`overflow-hidden rounded-[24px] ring-1 ${
      ok ? "bg-[#EAF8F0] ring-emerald-200/70" : "bg-[#F6FBF7] ring-[#0F2B20]/10"
    }`}>
      <div className="flex items-center gap-3 border-b border-[#0F2B20]/8 px-4 py-3">
        <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl ${
          ok ? "bg-[#059669]/15 text-[#059669]" : "bg-amber-100 text-amber-800"
        }`}>
          {icon}
        </span>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h3 className="font-display text-base font-black text-[#0F2B20]">{title}</h3>
            {ok && <CheckCircle2 size={15} className="text-[#059669]" />}
          </div>
        </div>
        <Link href={href} className="flex min-h-[34px] items-center gap-1.5 rounded-full bg-white px-3 text-xs font-black text-[#0F2B20] no-underline shadow-sm ring-1 ring-[#0F2B20]/10">
          {action}

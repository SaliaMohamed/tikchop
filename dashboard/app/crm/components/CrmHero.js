...
export function CrmHero({ stats, totalCustomers }) {
  return (
    <div className="relative mt-5 overflow-hidden rounded-[26px] bg-[#2b2219] p-5 text-white shadow-[0_4px_28px_rgba(43, 34, 25,0.25)]">
      <div className="absolute inset-x-0 top-0 h-1.5 bg-[#f0954c]" />
      <div className="relative z-10 flex items-start justify-between gap-4">
        <div>
          <p className="text-[0.68rem] font-black uppercase tracking-[0.14em] text-[#f0954c]/80">Carnet client</p>
        ...
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[18px] bg-white/10 text-[#f0954c] ring-1 ring-white/10">
      ...
      <div className="relative z-10 mt-5 rounded-[20px] bg-white/6 p-4 ring-1 ring-white/10">
        <div className="flex items-center gap-2 text-[0.68rem] font-black uppercase tracking-[0.12em] text-white/50">
          <Sparkles size={14} className="text-[#f0954c]" />
          <span>Ventes estimees</span>
        </div>
        <p className="mt-1 font-display text-2xl font-black text-[#f0954c]">{formatPrice(stats.estimatedSales)}</p>
      </div>
    </div>
  );
}
...
export function StatTile({ label, value, tone = "soft" }) {
  const className = tone === "dark"
    ...
    : tone === "green"
      ? "bg-[#fbefe2] text-[#96451f]"
      : "bg-white text-[#2b2219]";

  return (
    <div className={`p-3 text-center ${className}`}>
      <p className={`text-[0.62rem] font-black uppercase tracking-[0.1em] ${tone === "dark" ? "text-white/60" : tone === "green" ? "text-[#c2572b]" : "text-[#2b2219]/50"}`}>{label}</p>

...
export function CustomerSheet({ customer, sellerName, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[260] flex items-end bg-black/40 px-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))] backdrop-blur-sm md:items-center">
      <div className="mx-auto max-h-[92vh] w-full max-w-[480px] overflow-hidden rounded-[28px] bg-[#F6FBF7] shadow-2xl ring-1 ring-white/20">
        <div className="relative overflow-hidden bg-[#0F2B20] p-5 text-white">
          <div className="absolute inset-x-0 top-0 h-1.5 bg-[#34D399]" />
          <div className="relative z-10 flex items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="text-[0.68rem] font-black uppercase tracking-[0.14em] text-[#34D399]/80">Fiche client</p>
          ...
          <div className="relative z-10 mt-4 grid grid-cols-2 gap-2">
            <div className="rounded-[18px] bg-white/6 p-3 ring-1 ring-white/10">
              ...
              <p className="mt-0.5 font-display text-lg font-black text-[#34D399]">{formatPrice(total)}</p>
            </div>
            <div className="rounded-[18px] bg-white/6 p-3 ring-1 ring-white/10">
        ...
        <div className="no-scrollbar max-h-[58vh] space-y-4 overflow-y-auto p-4">
          <div className="rounded-[22px] bg-white p-3 ring-1 ring-[#0F2B20]/8">
            <div className="flex items-start gap-3">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#F6FBF7] text-[#059669] ring-1 ring-[#0F2B20]/5">
                {recommendation.icon}
              </span>
              <div>
                <p className="text-[0.62rem] font-black uppercase tracking-[0.14em] text-[#059669]">Action</p>
                <p className="mt-0.5 font-display text-base font-black leading-5 text-[#0F2B20]">{recommendation.title}</p>
                <p className="mt-0.5 text-xs font-bold leading-4 text-[#0F2B20]/60">{recommendation.body}</p>
              </div>
            </div>
          </div>
          ...
          <section>
            <div className="flex items-center gap-2 border-b border-[#0F2B20]/8 pb-2">
              <ShoppingBag size={14} className="text-[#059669]" />
              <p className="text-[0.68rem] font-black uppercase tracking-[0.12em] text-[#059669]">Derniere Commande</p>
            </div>
            <div className="mt-3 rounded-[22px] bg-white p-4 ring-1 ring-[#0F2B20]/8">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-display text-lg font-black text-[#0F2B20]">#{getOrderRef(lastOrder)}</p>
                  <p className="mt-0.5 truncate text-xs font-bold text-[#0F2B20]/60">{getItemsLabel(lastOrder)}</p>
                </div>
                <span className="shrink-0 rounded-full bg-[#F6FBF7] px-2.5 py-1 text-[0.65rem] font-black uppercase text-[#059669] ring-1 ring-[#0F2B20]/10">
                  {statusLabels[lastOrder?.status] || lastOrder?.status || "Statut"}
                </span>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2">
                <div className="rounded-[16px] bg-[#F6FBF7] p-2.5 ring-1 ring-[#0F2B20]/5">
                  <p className="text-[0.62rem] font-black uppercase text-[#0F2B20]/50">Total</p>
                  <p className="mt-0.5 font-display text-base font-black text-[#059669]">{formatPrice(getOrderTotal(lastOrder))}</p>
                </div>
                <div className="rounded-[16px] bg-[#F6FBF7] p-2.5 ring-1 ring-[#0F2B20]/5">
                  <p className="text-[0.62rem] font-black uppercase text-[#0F2B20]/50">Articles</p>
                  <p className="mt-0.5 font-display text-base font-black text-[#0F2B20]">{getItemCount(lastOrder)}</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <section>
            <div className="flex items-center gap-2 border-b border-[#0F2B20]/8 pb-2">
              <Clock3 size={14} className="text-[#059669]" />
              <p className="text-[0.68rem] font-black uppercase tracking-[0.12em] text-[#059669]">Historique</p>
            </div>
            <div className="mt-3 space-y-2">
              {customer.orders.map((order) => (
                <div key={order.id} className="grid grid-cols-[1fr_auto] items-center gap-3 rounded-[18px] bg-white p-3 ring-1 ring-[#0F2B20]/8">
                  <div className="min-w-0">
                    <p className="font-display text-sm font-black text-[#0F2B20]">#{getOrderRef(order)}</p>
                    <p className="mt-0.5 truncate text-[0.65rem] font-bold text-[#0F2B20]/50">{formatDate(order.created_at)} - {getItemsLabel(order)}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-display text-sm font-black text-[#059669]">{formatPrice(getOrderTotal(order))}</p>
                    <p className="mt-0.5 text-[0.62rem] font-black text-[#0F2B20]/40">{statusLabels[order.status] || order.status}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-2 border-t border-[#0F2B20]/8 bg-[#F6FBF7] p-3">
          <a
          ...
          <div className="grid grid-cols-2 gap-2">
            <a
              ...
              className={`flex min-h-[46px] items-center justify-center gap-2 rounded-[18px] text-xs font-black ${
                customer.phone ? "bg-white text-[#0F2B20] ring-1 ring-[#0F2B20]/10 shadow-sm" : "pointer-events-none bg-[#0F2B20]/5 text-[#0F2B20]/30"
              }`}
            >
              <Phone size={16} />
              Appeler
            </a>
            <button type="button" onClick={onClose} className="flex min-h-[46px] items-center justify-center rounded-[18px] bg-white text-xs font-black text-[#0F2B20] ring-1 ring-[#0F2B20]/10 shadow-sm">
              Fermer
            </button>
          </div>
        </div>

...
export default function SellerMenuPage() {
  ...
  if (!seller.slug) {
    return (
      <div className="app-shell pb-[calc(7rem+env(safe-area-inset-bottom,0px))] px-4">
        <div className="flex flex-col items-center justify-center text-center p-8 bg-[#2b2219] rounded-[32px] my-6 relative overflow-hidden text-white shadow-2xl">
          ...
          <div className="relative z-10 flex flex-col items-center max-w-xs">
            ...
            <Link
              ...
              className="mt-6 flex min-h-[54px] w-full items-center justify-center gap-2 rounded-[22px] bg-[#f0954c] text-base font-black text-[#2b2219] shadow-[0_16px_36px_rgba(240, 149, 76,0.3)] active:scale-[0.98] transition no-underline"
            >
              <Store size={18} />
              Créer ma boutique
            </Link>
  ...
  return (
    <div className="app-shell pb-[calc(7rem+env(safe-area-inset-bottom,0px))] px-4 space-y-3 pt-2">
      ...
      {/* Boutique card */}
      <div className="overflow-hidden rounded-[28px] bg-white shadow-[0_18px_50px_rgb(43_34_25_/_0.07)] ring-1 ring-[#2b2219]/8">
        <div className="h-1 w-full bg-gradient-to-r from-[#c2572b] via-[#f0954c] to-[#c2572b]" />
        <div className="flex items-center gap-3 px-4 pt-4 pb-3">
          <span className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-[18px] bg-[#2b2219] font-display text-base font-black text-[#f0954c]">
            {(seller.name || "T").slice(0, 1).toUpperCase()}
            <span className="absolute -bottom-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-[#f0954c] text-[#2b2219]">
              <Zap size={8} strokeWidth={3} />
            </span>
          </span>
          <div className="min-w-0 flex-1">
            <p className="text-[0.62rem] font-black uppercase tracking-[0.13em] text-[#c2572b]">Ma boutique</p>
            <h1 className="truncate font-display text-xl font-black text-[#2b2219]">{seller.name || "Boutique"}</h1>
          </div>
          <Link
            href={shopLink}
            className="flex h-10 w-10 items-center justify-center rounded-full bg-[#fbefe0] text-[#c2572b] no-underline"
          >
            <Eye size={17} />
          </Link>
        </div>
      ...
      {!whatsappConnected && hasProducts && (
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <small className="block text-xs font-bold text-[#6a5f50]">Vente auto inactive</small>
          </span>
          <ChevronRight size={17} className="text-[#c2572b] shrink-0" />
        </Link>
      )}
...
function MiniStat({ label, value, active = false, warn = false }) {
  return (
    <div className={`rounded-[16px] px-2 py-2.5 text-center ring-1 ${
      warn && !active ? "bg-[#fdf2d4] text-[#8a5c22] ring-[#f4c13a]/45"
      : active ? "bg-[#f0954c] text-[#2b2219] ring-[#f0954c]"
      : "bg-[#f4fbf7] text-[#2b2219] ring-[#2b2219]/6"
    }`}>
...
function MenuSection({ title, children }) {
  return (
    <section className="overflow-hidden rounded-[24px] bg-white shadow-[0_16px_40px_rgb(43_34_25_/_0.05)] ring-1 ring-[#2b2219]/8">
      <p className="px-4 pt-3 pb-1 text-[0.62rem] font-black uppercase tracking-[0.14em] text-[#c2572b]">{title}</p>
      <div className="divide-y divide-[#2b2219]/6">{children}</div>
    </section>
  );
}
...
function MenuItem({ href, icon, label, sub, accent = false, warn = false }) {
  return (
    <Link
      href={href}
      className={`grid min-h-[54px] grid-cols-[auto_1fr_auto] items-center gap-3 px-4 py-2 no-underline active:bg-[#f9f4ea] ${
        accent ? "bg-[#2b2219]" : warn ? "bg-[#fff9ea]" : "bg-white"
      }`}
    >
      <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl ${
        accent ? "bg-[#f0954c] text-[#2b2219]"
        : warn ? "bg-white text-[#9c651f] shadow-sm"
        : "bg-[#fbefe0] text-[#c2572b]"
      }`}>
        {icon}
      </span>
      <span className="min-w-0">
        <strong className={`block truncate text-sm font-black ${accent ? "text-white" : "text-[#2b2219]"}`}>{label}</strong>
        {sub && <small className={`block truncate text-xs font-bold ${accent ? "text-white/60" : "text-[#786d5e]"}`}>{sub}</small>}
      </span>
      <ChevronRight size={16} className={accent ? "text-[#f0954c]" : warn ? "text-[#9c651f]" : "text-[#c2572b]"} />
    </Link>
  );
}

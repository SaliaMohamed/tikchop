...
export default function SellerMenuPage() {
  ...
  if (!seller.slug) {
    return (
      <div className="app-shell pb-[calc(7rem+env(safe-area-inset-bottom,0px))] px-4">
        <div className="flex flex-col items-center justify-center text-center p-8 bg-[#0F2B20] rounded-[32px] my-6 relative overflow-hidden text-white shadow-2xl">
          ...
          <div className="relative z-10 flex flex-col items-center max-w-xs">
            ...
            <Link
              ...
              className="mt-6 flex min-h-[54px] w-full items-center justify-center gap-2 rounded-[22px] bg-[#34D399] text-base font-black text-[#0F2B20] shadow-[0_16px_36px_rgba(52, 211, 153,0.3)] active:scale-[0.98] transition no-underline"
            >
              <Store size={18} />
              Créer ma boutique
            </Link>
  ...
  return (
    <div className="app-shell pb-[calc(7rem+env(safe-area-inset-bottom,0px))] px-4 space-y-3 pt-2">
      ...
      {/* Boutique card */}
      <div className="overflow-hidden rounded-[28px] bg-white shadow-[0_18px_50px_rgb(43_34_25_/_0.07)] ring-1 ring-[#0F2B20]/8">
        <div className="h-1 w-full bg-gradient-to-r from-[#059669] via-[#34D399] to-[#059669]" />
        <div className="flex items-center gap-3 px-4 pt-4 pb-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[0.62rem] font-black uppercase tracking-[0.13em] text-[#059669]">Ma boutique</p>
            <h1 className="truncate font-display text-xl font-black text-[#0F2B20]">{seller.name || "Boutique"}</h1>
          </div>
          <Link
            href={shopLink}
      ...
      {!whatsappConnected && hasProducts && (
        <Link
          href="/setup"
          className="flex items-center gap-3 rounded-[22px] bg-[#E8F7EE] px-4 py-3 text-[#0F2B20] no-underline ring-1 ring-[#34D399]/35 active:scale-[0.99]"
        >
          <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#059669] text-white shrink-0">
            <Bot size={19} />
          </span>
          <span className="min-w-0 flex-1">
            <strong className="block text-sm font-black">Brancher WhatsApp</strong>
            <small className="block text-xs font-bold text-[#577066]">Vente auto inactive</small>
          </span>
          <ChevronRight size={17} className="text-[#059669] shrink-0" />
        </Link>
      )}
...
function MiniStat({ label, value, active = false, warn = false }) {
  return (
    <div className={`rounded-[16px] px-2 py-2.5 text-center ring-1 ${
      ...
      : active ? "bg-[#34D399] text-[#0F2B20] ring-[#34D399]"
      : "bg-[#F2FAF5] text-[#0F2B20] ring-[#0F2B20]/6"
    }`}>
...
function MenuSection({ title, children }) {
  return (
    <section className="overflow-hidden rounded-[24px] bg-white shadow-[0_16px_40px_rgb(43_34_25_/_0.05)] ring-1 ring-[#0F2B20]/8">
      <p className="px-4 pt-3 pb-1 text-[0.62rem] font-black uppercase tracking-[0.14em] text-[#059669]">{title}</p>
      <div className="divide-y divide-[#0F2B20]/6">{children}</div>
    </section>
  );
}
...
function MenuItem({ href, icon, label, sub, accent = false, warn = false }) {
  return (
    <Link
      href={href}
      className={`grid min-h-[54px] grid-cols-[auto_1fr_auto] items-center gap-3 px-4 py-2 no-underline active:bg-[#F5FBF8] ${
        accent ? "bg-[#0F2B20]" : warn ? "bg-[#fff9ea]" : "bg-white"
      }`}
    >
      <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl ${
        accent ? "bg-[#34D399] text-[#0F2B20]"
        : warn ? "bg-white text-[#9c651f] shadow-sm"
        : "bg-[#E8F7EE] text-[#059669]"
      }`}>
        {icon}
      </span>
      <span className="min-w-0">
        <strong className={`block truncate text-sm font-black ${accent ? "text-white" : "text-[#0F2B20]"}`}>{label}</strong>
        {sub && <small className={`block truncate text-xs font-bold ${accent ? "text-white/60" : "text-[#4E6E61]"}`}>{sub}</small>}
      </span>
      <ChevronRight size={16} className={accent ? "text-[#34D399]" : warn ? "text-[#9c651f]" : "text-[#059669]"} />
    </Link>
  );
}

          ...
          {orderTabs.map(({ key, label, icon: Icon }) => (
            <button
              ...
              className={`flex min-h-[44px] items-center gap-2 rounded-full px-3 text-sm font-black transition ${
                filter === key ? "bg-[#0F2B20] text-white" : "bg-white text-[#54685E] ring-1 ring-[#0F2B20]/8"
              }`}
            >
              <Icon size={16} />
      ...
      {error && (
        <div className="mt-4 rounded-[22px] bg-[#fdf3d6] p-3 text-xs font-black text-[#7a5425] ring-1 ring-[#f4ce60]/45">
          ...
          <div className="mt-3 flex gap-2">
            {sessionExpired && (
              <Link href="/login" className="inline-flex min-h-[38px] items-center justify-center rounded-full bg-[#0F2B20] px-4 text-xs font-black text-white no-underline">
                Connexion
              </Link>
            )}
            <button
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm font-semibold text-[#54685E] truncate">
            {demoOrder ? "Client test" : primaryLine}
          </p>
          <p className="mt-0.5 flex items-center gap-1 text-xs font-bold text-[#54685E]/50">
            <Package size={12} /> {itemCount} article{itemCount > 1 ? "s" : ""} ? {dateStr}
          </p>
        </div>
        <div className="text-right shrink-0">
          <p className="font-display text-base font-black text-[#0F2B20] whitespace-nowrap">
...
function OrderSheet({
  ...
  return (
    <div className="fixed inset-0 z-[260] flex items-end bg-[#0F2B20]/40 px-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))] backdrop-blur-sm md:items-center">
      <div className="animate-slide-up mx-auto max-h-[92vh] w-full max-w-[460px] overflow-hidden rounded-t-[32px] bg-white border border-[#DCEFE3]/45 shadow-2xl md:rounded-[32px]">
        <div className="relative overflow-hidden bg-[#F6FBF7] border-b border-[#DCEFE3]/40 p-5 text-[#0F2B20]">
          ...
          <div className="relative z-10 flex items-start justify-between gap-4">
            <div className="min-w-0 pr-12">
              <p className="text-[0.64rem] font-black uppercase tracking-[0.14em] text-[#54685E]/80">Commande #{order.order_ref || order.id?.slice(0, 8)}</p>
              <h2 className="mt-2 font-display text-xl font-black leading-7 text-[#0F2B20]">{nextAction.title}</h2>
              <p className="mt-1 text-xs font-semibold leading-relaxed text-[#54685E]">{nextAction.subtitle}</p>
            </div>
          </div>
          <div className="relative z-10 mt-4 grid grid-cols-2 gap-2">
            <div className="rounded-2xl bg-white border border-[#DCEFE3]/35 p-3 shadow-[0_2px_6px_rgba(58,47,30,0.02)]">
              <p className="text-[0.62rem] font-black uppercase tracking-wider text-[#54685E]/80">Total</p>
              <p className="mt-1 font-display text-lg font-black text-[#059669]">{formatPrice(total)}</p>
            </div>
            <div className="rounded-2xl bg-white border border-[#DCEFE3]/35 p-3 shadow-[0_2px_6px_rgba(58,47,30,0.02)]">
              <p className="text-[0.62rem] font-black uppercase tracking-wider text-[#54685E]/80">Articles</p>
              <p className="mt-1 font-display text-lg font-black text-[#0F2B20]">{getOrderItemCount(order)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="no-scrollbar max-h-[58vh] space-y-4 overflow-y-auto p-5">
          ...
          <div className="rounded-[24px] bg-[#F6FBF7] border border-[#DCEFE3]/45 p-4.5 text-[#0F2B20] shadow-[0_2px_10px_rgba(58,47,30,0.02)]">
            <div className="flex justify-between text-xs font-black uppercase tracking-wider text-[#54685E]">
              <span>Produits</span>
              <span>{formatPrice(order.total_amount)}</span>
            </div>
            <div className="mt-2.5 flex justify-between text-xs font-black uppercase tracking-wider text-[#54685E]">
              <span>Livraison</span>
              <span>{formatPrice(order.delivery_fee)}</span>
            </div>
            <div className="mt-3.5 flex justify-between border-t border-dashed border-[#DCEFE3] pt-3.5 text-lg font-black font-display">
              <span>Total</span>
        ...
        <div className="space-y-3 border-t border-zinc-100 p-4">
          ...
          {isCancelled ? (
            ...
            <button onClick={onPaid} className="flex min-h-[64px] w-full items-center justify-center gap-2 rounded-[22px] bg-[#059669] text-base font-extrabold text-white shadow-[0_14px_34px_rgba(0,143,90,0.15)] active:scale-[0.99] transition">
              <CheckCircle2 size={20} />
              Client confirme
            </button>
          ) : isPaid ? (
            <button onClick={onPrepared} className="flex min-h-[64px] w-full items-center justify-center gap-2 rounded-[22px] bg-[#0F2B20] text-base font-extrabold text-white shadow-[0_14px_34px_rgba(16,24,20,0.15)] active:scale-[0.99] transition">
              <Package size={20} />
              Marquer colis pret
            </button>
          ) : isInDelivery ? (
          ...
          <div className="grid grid-cols-2 gap-2">
            <a href={clientHref || undefined} target="_blank" rel="noopener noreferrer" className={`flex min-h-[52px] items-center justify-center gap-2 rounded-2xl text-xs font-black transition ${clientHref ? "bg-[#F6FBF7] text-[#0F2B20] border border-[#DCEFE3]/60 shadow-sm active:scale-98" : "pointer-events-none bg-zinc-100 text-zinc-400"}`}>
              <Send size={15} />
              Message client
            </a>
            <a href={receiptUrl} target="_blank" rel="noopener noreferrer" className="flex min-h-[52px] items-center justify-center gap-2 rounded-2xl bg-[#F6FBF7] text-xs font-black text-[#0F2B20] border border-[#DCEFE3]/60 shadow-sm transition active:scale-98">
              <ReceiptText size={15} />
              Recu
            </a>
          </div>
...
function SimpleOrderSheet({
  ...
  return (
    <div className="fixed inset-0 z-[260] flex items-end bg-[#0F2B20]/35 px-3 pb-[calc(0.7rem+env(safe-area-inset-bottom,0px))] backdrop-blur-sm md:items-center">
      <div className="animate-slide-up mx-auto max-h-[88vh] w-full max-w-[440px] overflow-hidden rounded-[30px] bg-[#F6FBF7] shadow-[0_28px_70px_rgb(43_34_25_/_0.28)] ring-1 ring-white/70">
        <header className="flex items-start justify-between gap-4 border-b border-[#0F2B20]/8 bg-white px-5 py-4">
          <div className="min-w-0">
            <p className="text-[0.62rem] font-black uppercase tracking-[0.14em] text-[#059669]">{statusLabels[status] || status}</p>
            <h2 className="mt-1 truncate font-display text-2xl font-black leading-7 text-[#0F2B20]">
              {order.order_ref || order.id?.slice(0, 8)?.toUpperCase()}
            </h2>
            <p className="mt-1 text-xs font-bold text-[#54685E]/60">{action.title}</p>
          </div>
          <button type="button" onClick={onClose} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#F6FBF7] text-[#0F2B20] ring-1 ring-[#0F2B20]/8" aria-label="Fermer">
            <X size={17} />
          </button>
        </header>

        <div className="no-scrollbar max-h-[58vh] space-y-3 overflow-y-auto px-4 py-4">
          <div className="rounded-[24px] bg-[#0F2B20] p-4 text-white">
            <div className="flex items-end justify-between gap-3">
              <span>
                <span className="block text-[0.62rem] font-black uppercase tracking-[0.14em] text-[#34D399]">Total</span>
          ...
          <section className="rounded-[22px] bg-white p-3 ring-1 ring-[#0F2B20]/7">
            <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.12em] text-[#059669]">
              <Package size={14} />
              Sachet
            </div>
            <div className="mt-3 space-y-2">
              {(items || []).map((item) => (
                <div key={item.id} className="flex items-center justify-between gap-3 rounded-[18px] bg-[#F6FBF7] px-3 py-2.5">
                  <span className="min-w-0">
                    <strong className="block truncate text-sm font-black text-[#0F2B20]">{item.products?.name || "Article"}</strong>
                    <small className="text-xs font-bold text-[#54685E]/55">A mettre dans le sachet</small>
                  </span>
                  <strong className="rounded-full bg-white px-3 py-1.5 text-sm font-black text-[#0F2B20] ring-1 ring-[#0F2B20]/6">x{item.quantity}</strong>
                </div>
              ))}
            </div>
          </section>
            ...
            <div className="no-scrollbar flex gap-2 overflow-x-auto">
              {availableDrivers.map((driver) => (
                <button
                  ...
                  className="min-h-[46px] shrink-0 rounded-full bg-white px-4 text-xs font-black text-[#0F2B20] ring-1 ring-[#0F2B20]/8"
                >
                  {driver.name}
                </button>
              ))}
        ...
        <footer className="space-y-2 border-t border-[#0F2B20]/8 bg-white p-4">
          <button
            type="button"
          ...
          <div className="grid grid-cols-3 gap-2">
            <a href={clientHref || undefined} target="_blank" rel="noopener noreferrer" className={`flex min-h-[46px] items-center justify-center rounded-[18px] text-xs font-black no-underline ring-1 ${clientHref ? "bg-[#F6FBF7] text-[#0F2B20] ring-[#0F2B20]/8" : "pointer-events-none bg-[#EDF6F0] text-[#54685E]/45 ring-transparent"}`}>
              Client
            </a>
            <button type="button" onClick={() => onShareDriver()} disabled={!canShareDriver} className="min-h-[46px] rounded-[18px] bg-[#F6FBF7] text-xs font-black text-[#0F2B20] ring-1 ring-[#0F2B20]/8 disabled:opacity-40">
              Livreur
            </button>
            <a href={receiptUrl} target="_blank" rel="noopener noreferrer" className="flex min-h-[46px] items-center justify-center rounded-[18px] bg-[#F6FBF7] text-xs font-black text-[#0F2B20] no-underline ring-1 ring-[#0F2B20]/8">
              Recu
            </a>
          </div>
          {!isDone && !isCancelled && (
...
function MiniInfo({ icon, label, value }) {
  return (
    <div className="flex items-center gap-3 rounded-[20px] bg-white px-3 py-3 ring-1 ring-[#0F2B20]/7">
      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#F6FBF7] text-[#059669]">
        {icon}
      </span>
      <span className="min-w-0">
        <small className="block text-[0.62rem] font-black uppercase tracking-[0.1em] text-[#54685E]/45">{label}</small>
        <strong className="block truncate text-sm font-black text-[#0F2B20]">{value}</strong>
      </span>
    </div>
  );
}
...
function OrderActionPath({ verifyCount, prepareCount, readyCount, deliveryCount, doneCount }) {
  const steps = [
    ...
    },
    {
      ...
      className: "bg-[var(--primary-bright)] text-[#091D14] ring-[var(--primary-bright)]",
    },
    {
      key: "driver",
      title: "Envoyer",
...
function OrderProgress({ status, deliveryStatus }) {
  ...
  return (
    <div className="rounded-[22px] bg-[#0F2B20] p-3.5 text-white shadow-md">
          ...
          <div key={step.key} className="flex flex-col items-center text-center">
            <span
              className={`flex h-9 w-9 items-center justify-center rounded-xl font-black text-xs transition-all ${
                step.active
                  ? "bg-[#34D399] text-[#0F2B20] shadow-[0_0_12px_rgba(52, 211, 153,0.45)]"
            ...
            <span
              className={`mt-1.5 text-[0.62rem] font-black uppercase tracking-tight ${
                step.active ? "text-[#34D399]" : "text-white/35"
              }`}
            >
              {step.label}
            </span>

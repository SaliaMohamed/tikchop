  ...
  return (
    <div className="flex min-h-dvh flex-col bg-[#f8f2e7] px-4 py-8">
      <div className="mx-auto w-full max-w-sm">
        ...
        <div className="mt-6">
          <p className="text-[0.68rem] font-black uppercase tracking-[0.14em] text-[#c2572b]">Étape 2 sur 2</p>
          <h1 className="mt-1 font-display text-3xl font-black text-[#2b2219]">Votre boutique</h1>
      ...
      <form onSubmit={onSubmit} className="mx-auto mt-6 w-full max-w-sm space-y-4">
        ...
        <div className="overflow-hidden rounded-[22px] bg-white ring-1 ring-[#2b2219]/8">
          <label htmlFor="onb-shop-name" className="flex min-h-[68px] cursor-text items-center gap-3 px-4 py-3">
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-[0.62rem] font-black uppercase tracking-[0.12em] text-[#c2572b]">Nom de la boutique</span>
              <input
                id="onb-shop-name"
                type="text"
                value={shopName}
            ...
            {shopName.trim().length >= 2 && <CheckCircle2 size={17} className="shrink-0 text-[#c2572b] ml-1" />}
          </label>
          {suggestedSlug && (
            <p className="px-4 pb-3 text-[0.65rem] font-bold text-[#2b2219]/40">
              Lien : tikchop.com/<span className="text-[#c2572b]">{suggestedSlug}</span>
            </p>
          )}
        </div>
        ...
        <div>
          <div className="flex items-center gap-2 px-1 mb-2">
            <Tag size={13} className="text-[#c2572b]" />
            <p className="text-[0.68rem] font-black uppercase tracking-[0.12em] text-[#c2572b]">Que vendez-vous ?</p>
          </div>
            ...
            {SHOP_CATEGORIES.map((cat) => (
              <button
                ...
                className={`flex items-center gap-2.5 rounded-[18px] px-3 py-3 text-left text-sm font-black transition active:scale-[0.98] ring-1 ${
                  shopCategory === cat.id
                    ? "bg-[#2b2219] text-[#f0954c] ring-[#2b2219]"
                    : "bg-white text-[#2b2219] ring-[#2b2219]/8 hover:ring-[#c2572b]/30"
                }`}
              >
        ...
        <div className="overflow-hidden rounded-[22px] bg-white ring-1 ring-[#2b2219]/8">
          <label htmlFor="onb-city" className="flex min-h-[58px] cursor-pointer items-center gap-3 px-4 py-3">
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-[0.62rem] font-black uppercase tracking-[0.12em] text-[#c2572b]">Ville principale (optionnel)</span>
              <select
                id="onb-city"
                value={shopCity}
                onChange={onShopCity}
                className="mt-0.5 w-full bg-transparent text-sm font-black text-[#2b2219] outline-none"
              >
